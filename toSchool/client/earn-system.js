...
export function getPlus1(e) {
  ...
  $("#delete_it").append(
    `<p class="v_text" style="position:absolute;color:red;left:${
      coord.x
    }px;top:${coord.y - 15}px;">+${localStorage.getItem('currentMoneyPerTap')}<p>`
  );
  ...
  $("#money_per_tap_text").text(
    `Money per tap: ${
      mySwitch2(String(localStorage.getItem("currentCowLevel")))
    } * ${Number(buster[1].bust) * Number(buster[2].bust)}`
  );

  $("#earn_button").css({'animation':'0.05s 1 alternate toUp'}, {'transform': 'translate(0px, -50px)'});
  setTimeout(()=>{
    $("#earn_button").css({'animation':'0.1s 1 alternate toDown'}, {'transform': 'translate(0px, 0px)'});
  }, 50);
}
...
setInterval(() => {
  localStorage.setItem(
    "currentMoney",
    `${Number(localStorage.getItem("currentMoney")) + Number(localStorage.getItem("currentInflows"))}`
  );
  $("#money_current_text").text(
    `Current money: ${localStorage.getItem("currentMoney")}`
  );
